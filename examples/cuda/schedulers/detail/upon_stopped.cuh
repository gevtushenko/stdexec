  ...
  class receiver_t
    ...
    void set_stopped() && noexcept {
      ...
      if constexpr (std::is_same_v<void, result_t>) {
        kernel<Fun><<<1, 1, 0, op_state_.stream_>>>(f_);
        ...
        kernel_with_result<Fun><<<1, 1, 0, stream>>>(f_, thrust::raw_pointer_cast(result.data()));
...
template <class SenderId, class ReceiverId, class Fun>
  struct op_state_t : operation_state_base_t {
    ...
    using Receiver = std::__t<ReceiverId>;
    using upon_stopped_receiver_t = receiver_t<ReceiverId, Fun>;
    using inner_op_state = std::execution::connect_result_t<Sender, upon_stopped_receiver_t>;
    ...
    op_state_t(Fun fn, Sender&& sender, Receiver receiver)
      : inner_op_{ 
          std::execution::connect(
              (Sender&&)sender, 
              upon_stopped_receiver_t((Receiver&&)receiver, fn, get_stream_provider())) } 
    { }
