...
namespace example::cuda::multi_gpu {
  ...
  using namespace example::cuda::stream;
  namespace detail = example::cuda::stream::detail;

  template <std::execution::sender Sender, std::integral Shape, class Fun>
    using bulk_sender_th = example::cuda::multi_gpu::bulk_sender_t<_P2300::__x<std::remove_cvref_t<Sender>>, Shape, _P2300::__x<std::remove_cvref_t<Fun>>>;
  ...
  struct scheduler_t {
    ...
    struct sender_t : sender_base_t {
      ...
      scheduler_t make_scheduler() const {
        return scheduler_t{num_devices_, hub_};
      }
      ...
      sender_t(int num_devices, detail::queue::task_hub_t* hub) noexcept
    ...
    template <std::execution::sender S, std::integral Shape, class Fn>
      ...
      tag_invoke(std::execution::bulk_t, const scheduler_t& sch, S&& sndr, Shape shape, Fn fun) noexcept {
        return bulk_sender_th<S, Shape, Fn>{{}, sch.num_devices_, (S&&) sndr, shape, (Fn&&)fun};
      }
    ...
    scheduler_t(int num_devices, const detail::queue::task_hub_t* hub)
      : num_devices_(num_devices)
